...
def get_embedding_feature(data, embedding_model):
    ...
    # print(embedding_model.wv['耳'])
    # print(embedding_model.wv['鼻'])
    # print(embedding_model.wv['耳鼻'])
    #对标签所有的字求embdding平均值
    w2v_label_embedding = np.array([
        np.mean([
            embedding_model.wv[word] for word in key
...
def wam(sentence, w2v_model, method='mean', aggregate=True):
    arr = np.array([
        w2v_model.wv[s] for s in sentence
...
def Find_Label_embedding(example_matrix, label_embedding, method='mean'):
    ...
    # 根据矩阵乘法来计算label与word之间的相似度 seqlen,embeding_dim*(labellen,embedding_dim).T=seqlen,labellen
    ...
    # 将样本的词嵌入与注意力机制相乘得到，标签得分更大的字得到更多的关注
    attention_embedding = example_matrix * attention#seqlen,embeding_dim
